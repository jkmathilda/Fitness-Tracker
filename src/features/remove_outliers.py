...
df[outlier_columns[:3] + ['label']].boxplot(by='label', figsize=(20, 10))
df[outlier_columns[3:] + ['label']].boxplot(by='label', figsize=(20, 10))
...
for col in outlier_columns:
    dataset, outliers, X_scores = mark_outliers_lof(df, [col], n=20)
...
for col in outlier_columns:
    dataset, outliers, X_scores = mark_outliers_lof(df[df['label'] == label], [col], n=20)
    plot_binary_outliers(dataset, col, 'outlier_lof', reset_index=True)
...
dataset[dataset['gyr_z_outlier'] == True]   # Outliers
dataset.loc[dataset['gyr_z_outlier'] == True, 'gyr_z'] = np.nan  # Set outliers to NaN
